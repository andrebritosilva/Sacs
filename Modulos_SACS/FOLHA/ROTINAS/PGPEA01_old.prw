...
aPdv        := {}
aPd         := {}
_cProcess   := "00001" //Add BY MVCARD 20181002
_cPeriodo	:= ""    
...
While TRB->(!Eof())
	...
	If DbSeek(xFilial("RCH")+_cProcess+c__Roteiro+"1" )
    ...
    DbSelectArea("RGB")
    DbSetorder(1)
    If !DbSeek(xFilial("RGB")+TRB->MAT+cVerba )
       RecLock("RGB",.T.)
       ...
       MsUnlock()*/
		...
		RGB->RGB_HORAS  := 0		
		RGB->RGB_PARCEL := 0
		RGB->RGB_PROCES := "00001" //PROCESSO 
		RGB->RGB_ROTEIR := "FOL" //ROTEIRO
		RGB->RGB_PERIOD := _cPeriodo //Periodo
