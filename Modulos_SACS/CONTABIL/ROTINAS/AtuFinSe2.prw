...
User Function AtuFinSe2()
	...
	oModCT1Imp	:= FWLoadModel("FINA050")
...
oProcess:IncRegua2("Alterando o título: " + aDados[i][2] + "" + Str(i))

cCod := Padr(aDados[i][3],TamSx3('B1_COD')[1]) 

cFilTit  := Strzero(Val(aDados[i][1]),TamSx3("E2_FILIAL")[1])
cPrefixo := Padr(aDados[i][4],TamSx3('E2_PREFIXO')[1])
cNum     := Strzero(Val(aDados[i][2]),TamSx3("E2_NUM")[1])
cParcela := Padr(aDados[i][11],TamSx3('E2_PARCELA')[1])
cTipo    := Padr(aDados[i][3],TamSx3('E2_TIPO')[1])

If DbSeek( xFilial("SE2") + cPrefixo + cNum + cParcela + cTipo)
	
	RecLock("SE2",.F.)
		
		SE2->E2_ITEMD    := Alltrim(aDados[i][6])
		SE2->E2_CCD      := Alltrim(aDados[i][7])
...
User Function Avc01Msg(nDados, nInclu, aProdutos)
