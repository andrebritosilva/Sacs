...
User Function MT120FIM()
	...
	Private cLinkDoc := SuperGetMV("SC_DIRDOC",.T.,"VA8PF4.prd.protheus.totvscloud.com.br:31148/dirdoc")  //diretorio onde se encontram os documentos que irao acompanhar o pedido
		...
		If nVlrPed < 300
			...
			If SC7->C7_COND == "CXA"
				...
				While SCR->(!Eof()) .And. Alltrim(SCR->CR_NUM) == SC7->C7_NUM
					...
					If SCR->CR_APROV # "000004"
						If SCR->CR_APROV # "000037"
							DbSkip()
							Loop
						Else
							lAprovCxa := .T.
	     ...
	     //Verifico se existe somente um aprovador no grupo 
	      
				If ! DbSeek(xFilial("SCR")+"PC"+SC7->C7_NUM ) .Or. !lAprovCxa
	  
					MaAlcDoc({SC7->C7_NUM,"PC",nTotPed,,,"999903",,SC7->C7_MOEDA,SC7->C7_TXMOEDA,SC7->C7_EMISSAO},,1)
			...
			Else
				...
				If ! DbSeek(xFilial("SCR")+"PC"+SC7->C7_NUM ) //
					...
					MaAlcDoc({SC7->C7_NUM,"PC",nTotPed,,,"999902",,SC7->C7_MOEDA,SC7->C7_TXMOEDA,SC7->C7_EMISSAO},,1)
...
//					MsUnlock()
      ...
      //Qdo nao achar o aprovador de Caixa
				If !lAprovCxa
...
cQuery2 += " C7_CC,C7_ITEMCTA, C7_FRETE, C7_DESPESA, C7_VALFRE, C7_DESC, CR_USER, CR_NIVEL, CR_APROV, CR_USERLIB, CR_LIBAPRO, CR_USERORI, CR_APRORI, CR_TOTAL "
...
While !(cAliasTop2)->(EOF())
	...
	//aadd(oWF:oHtml:ValByName("t1.cItem"  ), (cALIASTOP2)->C7_ITEMCTA)
	aadd(oWF:oHtml:ValByName("t1.cCusto"  ), (cALIASTOP2)->C7_ITEMCTA)
...
For j = 1 to len(aProdutos)
	...
	cQuery3 := " SELECT TOP 3 C7_FILIAL, C7_ITEM, C7_PRODUTO, C7_NUM, C7_FORNECE, C7_LOJA, A2_NOME, C7_QUANT, C7_CC, C7_ITEMCTA , "
...
//			Aadd(aProdutos,{ (cALIASTOP2)->C7_PRODUTO} )
	...
	For j := 1 to len(aProdutos)
		...
		cQuery3 := " SELECT TOP 3 C7_FILIAL, C7_ITEM, C7_PRODUTO, C7_NUM, C7_FORNECE, C7_LOJA, A2_NOME, C7_QUANT, C7_ITEMCTA, C7_CC, "
		cQuery3 += " C7_PRECO, C7_IPI , C7_EMISSAO, C7_DATPRF, C7_COND, E4_DESCRI, C7_QUJE "
		cQuery3 += " FROM "+retsqlname("SC7")+" C7 "
		cQuery3 += " INNER JOIN "+retsqlname("SA2")+" A2 ON "
		cQuery3 += " 	C7_FORNECE = A2_COD AND "
	...
	If !lPedidos
		...
		aadd(oWF:oHtml:ValByName("t2.nIPI"      ),"--")
		aadd(oWF:oHtml:ValByName("t2.dEmissao"  ),"--")
		//aadd(oWF:oHtml:ValByName("t2.cItem"  ),"--")
		aadd(oWF:oHtml:ValByName("t2.cCusto"  ),"--")
		aadd(oWF:oHtml:ValByName("t2.cPgto"     ),"--")
	...
	Else
		...
		For j := 1 To Len(aSC7)
			...
			aadd(oWF:oHtml:ValByName("t2.nIPI"      ),aSC7[j][6])
			aadd(oWF:oHtml:ValByName("t2.dEmissao"  ),aSC7[j][7])
			//aadd(oWF:oHtml:ValByName("t2.cItem"  ),aSC7[j][8])
			aadd(oWF:oHtml:ValByName("t2.cCusto"  ),aSC7[j][8])
			aadd(oWF:oHtml:ValByName("t2.cPgto"     ),aSC7[j][9])
			cItem := aSC7[j][1]
		Next
...
User Function WfPcRet(oRet)
	...
	Private cLinkArq := SuperGetMV("SC_DIRDOC",.T.,"VA8PF4.prd.protheus.totvscloud.com.br:31148/dirdoc")
	cPath   := "/co01/shared/"
	...
	If cAprova = "1"
		...
		If !EMPTY((cALIASTOP1)->CR_NUM)
			...
			cQuery4 += " C7_CC,C7_ITEMCTA, C7_FRETE, C7_DESPESA, C7_VALFRE, C7_DESC, CR_USER, CR_APROV,CR_USERLIB, CR_LIBAPRO, CR_USERORI, CR_APRORI,CR_TOTAL "
		...
		//para enviar copia de email
			...
			For j = 1 to len(aProdutos)
				...
				cQuery6 := " SELECT TOP 3 C7_FILIAL, C7_ITEM, C7_PRODUTO, C7_NUM, C7_FORNECE, C7_LOJA, A2_NOME, C7_QUANT,C7_ITEMCTA, C7_CC ,"
		...
		Else
			...
			cQuery44 += " C7_CC, C7_ITEMCTA, C7_FRETE, C7_DESPESA, C7_VALFRE, C7_DESC, CR_USER, CR_APROV, CR_USERLIB, CR_LIBAPRO, CR_USERORI, CR_APRORI, CR_TOTAL "
		...
		//oWF:cBCC := cEmailCo
			...
			While !(cAliasTop12)->(EOF())
				...
				//aadd(oWF:oHtml:ValByName("t1.cItem"  ), (cALIASTOP12)->C7_ITEMCTA)
				aadd(oWF:oHtml:ValByName("t1.cCusto"  ), (cALIASTOP12)->C7_ITEMCTA)
