...
User Function ConsSC7()
	...
	Local aButtons  := {}
	Local nOpca     := 0
	Local cCadastro := OemToAnsi("Gera o consulta por C.Custo dos Pedidos Compras")
...
//Faco a abertura do SX2 da empresa que ira gerar o arquivo de trabalho
	...
	cQuery := " SELECT  SC7.C7_NUM, SC7.C7_EMISSAO, SC7.C7_CC, SC7.C7_TOTAL, SC7.C7_FORNECE, SC7.C7_LOJA,SC7.C7_OBS, SC7.C7_QTDACLA,  "
	...
	cQuery += "         ISNULL(SD1.D1_SERIE,' ') AS D1_SERIE,ISNULL(SD1.D1_QUANT,0) AS D1_QUANT,ISNULL(SD1.D1_TOTAL,0) AS D1_TOTAL,SA2.A2_NOME, CTT.CTT_DESC01  "
	...
	cQuery += "	And SC7.C7_CC  BETWEEN '"+MV_PAR07+"' And '"+mv_par08+"'"
...
//oPrn:SetPortrait()
	...
	//ภฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤู
		...
		AaDD(aBrowse,{"MOTIVO","","Motivo",""})
				...
				oPrn:Say(nLin+10, 2960,TRB->CC         ,oFont9 ,100)
