...
User Function XmlMDFeSef(cFil)
	...
	If !Empty(aNota)
		cString := ""
		cString += MDFeIde(@cChvMDFe,aNota)
		cString += MDFeEmit()
...
/*/
//-----------------------------------------------------------------------
Static Function MDFeIde(cChave,aNota)
	...
	For nX := 1 To Len(aVeiSF2)
		If HasTemplate("DCLEST")	//ExistTemplate("OMSA200P")
			...
			If MsSeek(xFilial("LBW")+PADR(aVeiSF2[nX],Len(LBW->LBW_PLACA)))
				aadd(aVeiculo,{})
				aadd(aVeiculo[Len(aVeiculo)],LBW->LBW_PLACA)					
				aadd(aVeiculo[Len(aVeiculo)],LBW->LBW_PLACA) 
				aadd(aVeiculo[Len(aVeiculo)],LBW->LBW_UF)
				aadd(aVeiculo[Len(aVeiculo)],LBW->LBW_TARA)
				aadd(aVeiculo[Len(aVeiculo)],"")
				aadd(aVeiculo[Len(aVeiculo)],LBW_TRANSP)
				aadd(aVeiculo[Len(aVeiculo)],"")
				aadd(aVeiculo[Len(aVeiculo)],LBW->LBW_FROVEI) //Frota 1-Própria;2-Terceiro;3-Agregado
				aadd(aVeiculo[Len(aVeiculo)],LBW->LBW_CAPTOT) //Capacidade Máxima
				aadd(aVeiculo[Len(aVeiculo)],LBW->LBW_ALTINT) //Altura Interna
				aadd(aVeiculo[Len(aVeiculo)],LBW->LBW_LARINT) //Largura Interna
				aadd(aVeiculo[Len(aVeiculo)],LBW->LBW_COMINT) //Comprimento Interno
				aadd(aVeiculo[Len(aVeiculo)],LBW->LBW_TIPROD ) 	//Tipo de Rodado
				aadd(aVeiculo[Len(aVeiculo)],LBW->LBW_TIPCAR ) 	//Tipo de Carroceria
				aadd(aVeiculo[Len(aVeiculo)],LBW->LBW_RENAVA) //Renavam
				...
				If nX == 1
					...
					aadd(aMotorista[Len(aMotorista)],LBW->LBW_NOMMOT)
					aadd(aMotorista[Len(aMotorista)],LBW->LBW_CPFMOT)
				EndIf
			Else
				dbSelectArea("DA3")
				dbSetOrder(1)
				...
				If nX == 1
					aadd(aMotorista,{})
					If !Empty(cMotorista) //aVeiculo[Len(aVeiculo)][7])
						dbSelectArea("DA4")
						dbSetOrder(1)
						MsSeek(xFilial("DA4")+cMotorista ) //aVeiculo[Len(aVeiculo)][7])
		...
		Else
			...
			aadd(aVeiculo[Len(aVeiculo)],DA3->DA3_LOJFOR)
			aadd(aVeiculo[Len(aVeiculo)],DA3->DA3_CODFOR)
			aadd(aVeiculo[Len(aVeiculo)],CC0->CC0_MOTORI)
			aadd(aVeiculo[Len(aVeiculo)],DA3->DA3_FROVEI) //Frota 1-Própria;2-Terceiro;3-Agregado
			aadd(aVeiculo[Len(aVeiculo)],DA3->DA3_CAPACM) //Capacidade Máxima
			aadd(aVeiculo[Len(aVeiculo)],DA3->DA3_ALTINT) //Altura Interna
			aadd(aVeiculo[Len(aVeiculo)],DA3->DA3_LARINT) //Largura Interna
